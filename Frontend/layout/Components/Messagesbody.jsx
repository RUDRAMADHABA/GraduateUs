...
const ChatPage = (props) => {
    ...
    const fetchMessages = async () => {
      try {
        ...
        const response = await axios.get(
          `https://graduate-us-backend-gkli.onrender.com/message/${chatId}`,
          { headers }
        );
  ...
  return (
    <Box sx={{ maxWidth: 800, padding: 4 }} marginTop={{sm:"105px",md:"125px"}} marginLeft={{sm:"106px",md:"303px"}}>
         <Link href='/chat'><Typography sx={{ marginBottom: 2 ,color:props.letter,fontStyle:"italic",display:"flex",justufyContent:"center",alignItems:"center",gap:"5px"}} > <KeyboardBackspaceIcon sx={{fontSize:"16px"}}/> Back to chats</Typography></Link>
      ...
      >
        {messages.map((message, index) => (
          ...
          sx={{
            ...
            flexDirection: message.sender._id === session.user._id ? 'row-reverse' : 'row',
            marginBottom: 2,
          }}
        >
          {index === 0 || messages[index - 1].sender._id !== message.sender._id ? (
            <Box
              ...
              sx={{
                ...
                alignItems: message.sender._id === session.user._id ? 'flex-end' : 'flex-start',
            ...
            <Box
              sx={{
                ...
                backgroundColor: message.sender._id === session.user._id ? '#DCF8C6' : '#F0F0F0',
                marginLeft: 2,
                marginRight: 2,
              }}
            >
              <Box sx={{ marginLeft: 2 }}>
                {index === 0 || messages[index - 1].sender._id !== message.sender._id ? (
                  <Typography
                    variant="subtitle2"
                    sx={{
                      color: message.sender._id === session.user._id ? 'green' : 'blue',
                    }}
                  >
...
/>
        ...
        <Button variant="contained" onClick={sendMessage} sx={{ marginTop: 2 ,color:props.letter1,background:props.button,'&:hover':{
